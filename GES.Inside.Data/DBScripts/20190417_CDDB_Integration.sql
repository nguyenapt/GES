IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'PeerGroups')
	SET NOEXEC ON

CREATE TABLE [dbo].[PeerGroups] (
    [Id] [int] NOT NULL,
    [Name] [nvarchar](100) NOT NULL,
    CONSTRAINT [PK_dbo.PeerGroups] PRIMARY KEY ([Id])
)

CREATE TABLE [dbo].[SubPeerGroups] (
    [Id] [int] NOT NULL,
    [Name] [nvarchar](100) NOT NULL,
	[PeerGroupId] [int] NOT NULL,
    CONSTRAINT [PK_dbo.SubPeerGroups] PRIMARY KEY ([Id]), 
	CONSTRAINT [FK_dbo.SubPeerGroups_dbo.PeerGroups_PeerGroupId] FOREIGN KEY ([PeerGroupId]) REFERENCES [dbo].[PeerGroups] ([Id])
)
CREATE INDEX [IX_PeerGroupId] ON [dbo].[SubPeerGroups]([PeerGroupId])

CREATE TABLE [dbo].[Regions] (
    [Id] [uniqueidentifier] NOT NULL,
    [Name] [nvarchar](50) NOT NULL,
    CONSTRAINT [PK_dbo.Regions] PRIMARY KEY ([Id])
)
...
CREATE TABLE [dbo].[Countries] (
	...
	CONSTRAINT [FK_dbo.Countries_dbo.Regions_RegionId] FOREIGN KEY ([RegionId]) REFERENCES [dbo].[Regions] ([Id])
)
CREATE INDEX [IX_RegionId] ON [dbo].[Countries]([RegionId])

ALTER TABLE dbo.I_Companies ADD Id int NOT NULL CONSTRAINT DF_I_Companies_Id DEFAULT 0
ALTER TABLE dbo.I_Companies ADD InternalId uniqueidentifier NOT NULL CONSTRAINT DF_I_Companies_InternalId DEFAULT NEWID()
ALTER TABLE dbo.I_Companies ADD Name nvarchar(100) NOT NULL CONSTRAINT DF_I_Companies_Name DEFAULT ''
ALTER TABLE dbo.I_Companies ADD ExchangeTicker nvarchar(50) 
ALTER TABLE dbo.I_Companies ADD CountryOfIncorporationId uniqueidentifier CONSTRAINT [FK_dbo.I_Companies_dbo.Countries_CountryOfIncorporationId] FOREIGN KEY ([CountryOfIncorporationId]) REFERENCES [dbo].[Countries] ([Id])
ALTER TABLE dbo.I_Companies ADD SubPeerGroupId int CONSTRAINT [FK_dbo.I_Companies_dbo.SubPeerGroups_SubPeerGroupId] FOREIGN KEY ([SubPeerGroupId]) REFERENCES [dbo].[SubPeerGroups] ([Id])
ALTER TABLE dbo.I_Companies ADD Website nvarchar(255) 
ALTER TABLE dbo.I_Companies ADD IsParkedForGssResearchSince datetime2
GO

UPDATE I_Companies SET Id = I_Companies_Id, Name = ISNULL(CAST(NameCoalesce as nvarchar(100)), ''), Website = WebsiteUrl

ALTER TABLE dbo.I_Companies DROP CONSTRAINT DF_I_Companies_Id
ALTER TABLE dbo.I_Companies DROP CONSTRAINT DF_I_Companies_InternalId
ALTER TABLE dbo.I_Companies DROP CONSTRAINT DF_I_Companies_Name

CREATE UNIQUE INDEX [IX_I_Companies_Id] ON [dbo].[I_Companies]([Id])
CREATE INDEX [IX_I_Companies_ExchangeTicker] ON [dbo].[I_Companies]([ExchangeTicker])
CREATE INDEX [IX_I_Companies_CountryOfIncorporationId] ON [dbo].[I_Companies]([CountryOfIncorporationId])
CREATE INDEX [IX_I_Companies_SubPeerGroupId] ON [dbo].[I_Companies]([SubPeerGroupId])
CREATE INDEX [IX_I_Companies_IsParkedForGssResearchSince] ON [dbo].[I_Companies]([IsParkedForGssResearchSince])

GO
ALTER TABLE dbo.I_GesCaseReports ADD CountryId uniqueidentifier CONSTRAINT [FK_dbo.I_GesCaseReports_dbo.Countries_CountryId] FOREIGN KEY ([CountryId]) REFERENCES [dbo].[Countries] ([Id])
ALTER TABLE dbo.I_NaArticles ADD CountryId uniqueidentifier CONSTRAINT [FK_dbo.I_NaArticles_dbo.Countries_CountryId] FOREIGN KEY ([CountryId]) REFERENCES [dbo].[Countries] ([Id])
ALTER TABLE dbo.G_Organizations ADD CountryId uniqueidentifier CONSTRAINT [FK_dbo.G_Organizations_dbo.Countries_CountryId] FOREIGN KEY ([CountryId]) REFERENCES [dbo].[Countries] ([Id])
GO
CREATE INDEX [IX_I_GesCaseReports_CountryId] ON [dbo].[I_GesCaseReports]([CountryId])
CREATE INDEX [IX_I_NaArticles_CountryId] ON [dbo].[I_NaArticles]([CountryId])
CREATE INDEX [IX_G_Organizations_CountryId] ON [dbo].[G_Organizations]([CountryId])

	SET NOEXEC OFF


	GO
IF EXISTS (SELECT * FROM   sys.objects WHERE  object_id = OBJECT_ID(N'[dbo].[splitToGuid]') AND type IN ( N'FN', N'IF', N'TF', N'FS', N'FT' ))
begin
	DROP FUNCTION [dbo].[splitToGuid]
end

GO
CREATE FUNCTION [dbo].[splitToGuid]
    (
      @values VARCHAR(8000) ,
      @delimiter VARCHAR(10)
    )
RETURNS @result TABLE (value UNIQUEIDENTIFIER)
AS
    BEGIN
        DECLARE @v AS VARCHAR(8000);
		DECLARE @temp AS UNIQUEIDENTIFIER;
        ...
        WHILE CHARINDEX(@delimiter, @values) <> 0
            BEGIN
				...
				SET @temp  = TRY_CAST(@v AS UNIQUEIDENTIFIER);
		...
		SET @temp  = TRY_CAST(@values AS UNIQUEIDENTIFIER);
