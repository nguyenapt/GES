...
DECLARE ungpResourceCur CURSOR FOR SELECT ColumnName, OldValue, NewValue, AuditDataState FROM @GesUNGPAssessmentFormAuditColumnsList
...
FETCH NEXT FROM ungpResourceCur INTO @ColumnName, @OldValue, @NewValue, @AuditDataState

WHILE @@FETCH_STATUS = 0 BEGIN
    insert into GesUNGPAssessmentFormResource_Audit_Details(GesUNGPAssessmentFormResource_Audit_Details_Id, GesUNGPAssessmentFormResource_Audit_Id, ColumnName, OldValue ,NewValue, AuditDataState, AuditUser, AuditDatetime)
	Values(NEWID(), @GesUNGPAssessmentFormResourceAuditId, @ColumnName, @OldValue, @NewValue, @AuditDataState, @AuditUser, GETDATE())
