...
DECLARE ungpCur CURSOR FOR SELECT ColumnName, ColumnNameDescription, OldValue, NewValue, AuditDataState FROM @GesUNGPAssessmentFormAuditColumnsList
...
FETCH NEXT FROM ungpCur INTO @ColumnName,@ColumnNameDescription, @OldValue, @NewValue, @AuditDataState

WHILE @@FETCH_STATUS = 0 BEGIN
    insert into GesUNGPAssessmentForm_Audit_Details(GesUNGPAssessmentForm_Audit_Details_Id, GesUNGPAssessmentForm_Audit_Id, ColumnName,ColumnNameDescription, OldValue ,NewValue, AuditDataState, AuditUser, AuditDatetime)
	Values(NEWID(), @GesUNGPAssessmentFormAuditId, @ColumnName,@ColumnNameDescription, @OldValue, @NewValue, @AuditDataState, @AuditUser, GETDATE())
