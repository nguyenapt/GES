...
BEGIN
	...
	WHERE gcr.ShowInClient = 1 
		  ...
		  AND (@isGovernanceTypeSearch = 0 OR (@NormId = 1 AND (LOWER(RTRIM(LTRIM(gcr.ReportIncident))) LIKE 'pre-agm engagement%' OR LOWER(RTRIM(LTRIM(gcr.ReportIncident))) LIKE 'agm-related%')) OR (@NormId = 2 AND LOWER(RTRIM(LTRIM(gcr.ReportIncident))) LIKE 'ongoing engagement%'))
