ALTER PROCEDURE [dbo].[CompanyFocusList]
	@OrgId BIGINT,
	@IndividualId BIGINT
AS
BEGIN
...
MERGE INTO I_GesCaseReports T
   USING (
          SELECT r.I_GesCaseReports_Id, ct2.Id FROM dbo.I_GesCaseReports r
INNER JOIN dbo.G_Countries ct ON ct.G_Countries_Id = r.LocationG_Countries_Id
INNER JOIN dbo.Countries ct2 ON ct.IsoAlpha3 = ct2.Alpha3Code
WHERE r.LocationG_Countries_Id IS NOT NULL
         ) S
      ON T.I_GesCaseReports_Id = S.I_GesCaseReports_Id
WHEN MATCHED THEN
   UPDATE 
      SET T.CountryId = S.Id;

ENABLE TRIGGER dbo.I_GesCaseReports_Update ON dbo.I_GesCaseReports


GO
MERGE INTO dbo.I_Companies T
   USING (
          SELECT r.I_Companies_Id, ct2.Id FROM dbo.I_Companies r
INNER JOIN dbo.G_Countries ct ON ct.G_Countries_Id = r.CountryIncG_Countries_Id
INNER JOIN dbo.Countries ct2 ON ct.IsoAlpha3 = ct2.Alpha3Code
WHERE r.CountryIncG_Countries_Id IS NOT NULL
         ) S
      ON T.I_Companies_Id = S.I_Companies_Id
WHEN MATCHED THEN
   UPDATE 
      SET T.CountryOfIncorporationId = S.Id;


GO

MERGE INTO dbo.I_NaArticles T
   USING (
          SELECT r.I_NaArticles_Id, ct2.Id FROM dbo.I_NaArticles r
INNER JOIN dbo.G_Countries ct ON ct.G_Countries_Id = r.G_Countries_Id
INNER JOIN dbo.Countries ct2 ON ct.IsoAlpha3 = ct2.Alpha3Code
WHERE r.G_Countries_Id IS NOT NULL
         ) S
      ON T.I_NaArticles_Id = S.I_NaArticles_Id
WHEN MATCHED THEN
   UPDATE 
      SET T.CountryId = S.Id;
...
MERGE INTO dbo.G_Organizations T
   USING (
          SELECT r.G_Organizations_Id, ct2.Id FROM dbo.G_Organizations r
INNER JOIN dbo.G_Countries ct ON ct.G_Countries_Id = r.G_Countries_Id
INNER JOIN dbo.Countries ct2 ON ct.IsoAlpha3 = ct2.Alpha3Code
WHERE r.G_Countries_Id IS NOT NULL
         ) S
      ON T.G_Organizations_Id = S.G_Organizations_Id
WHEN MATCHED THEN
   UPDATE 
      SET T.CountryId = S.Id;
