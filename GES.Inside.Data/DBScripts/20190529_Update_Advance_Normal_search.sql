...
BEGIN
	...
	IF @PortfolioIds IS NOT NULL AND LEN(@PortfolioIds) > 0 BEGIN
		...
		WHERE po.G_Organizations_Id = @orgId 
			...
			--AND (@CompanyIssueName IS NULL OR c.Name LIKE '%' + @CompanyIssueName + '%')
...
BEGIN
	...
	BEGIN
		...
		AND c.Id >= 1000000000
		AND (@Isin IS NULL OR c.Isin LIKE '%' + @Isin + '%')
	AND (@SustainalyticsId IS NULL OR c.Id = @SustainalyticsId)
	END
	ELSE
		INSERT INTO #tempFilteredCompanies
		SELECT CASE WHEN c.MasterI_Companies_Id IS NOT NULL THEN c.MasterI_Companies_Id ELSE c.I_Companies_Id END AS  I_Companies_Id,
	...
	CREATE TABLE #tempMainQuery (Id BIGINT, CompanyId BIGINT, CompanyIssueName NVARCHAR(255),CompanyName NVARCHAR(255), HomeCountry NVARCHAR(255), Isin VARCHAR(50), Sedol VARCHAR(50), MarketCap DECIMAL, NumCases INT NULL, NumAlerts INT NULL, IsInFocusList INT NULL, CountryId UNIQUEIDENTIFIER NULL, SubPeerGroupId INT NULL, SustainalyticsID  int NOT NULL)
	...
	DECLARE @MainQuery TABLE (Id IId, CompanyId IId, CompanyIssueName Name,CompanyName Name, HomeCountry NVARCHAR(255), Isin VARCHAR(50), Sedol VARCHAR(50), MarketCap DECIMAL, NumCases INT NULL, NumAlerts INT NULL, IsInFocusList INT NULL, CountryId UNIQUEIDENTIFIER NULL, SubPeerGroupId int NULL, SustainalyticsID  int NULL)
	...
	JOIN(
		...
		WHERE tc.ShowInClient = 1
		AND (tc.I_NormAreas_Id IS NULL or tc.I_NormAreas_Id <> 6)
		AND (@normId IS NULL OR @isGovernanceTypeSearch = 1 OR tc.I_NormAreas_Id = @normId)
