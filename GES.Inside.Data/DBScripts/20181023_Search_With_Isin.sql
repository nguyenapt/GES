...
/****** Object:  StoredProcedure [dbo].[CompanyAdvancedSearch]    Script Date: 10/23/2018 3:23:37 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		INNER JOIN (
			...
			AND (@CompanyIssueName IS NULL OR c.NameCoalesce LIKE '%' + @CompanyIssueName + '%' OR c.Isin = @CompanyIssueName OR tc.ReportIncident LIKE '%' + @CompanyIssueName + '%' OR grce.Keywords LIKE '%' + @CompanyIssueName + '%' )
...
/****** Object:  StoredProcedure [dbo].[CompanyNormalSearch]    Script Date: 10/23/2018 1:42:11 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
	...
	AND c.NameCoalesce IS NOT NULL 
	AND (c.MasterI_Companies_Id IS NULL OR c.MasterI_Companies_Id = c.I_Companies_Id)
	AND (@CompanyIssueName IS NULL OR c.NameCoalesce LIKE '%' + @CompanyIssueName + '%' OR c.Isin = @CompanyIssueName OR gc.I_GesCompanies_Id IN (SELECT * FROM @tempCompanyIds))
