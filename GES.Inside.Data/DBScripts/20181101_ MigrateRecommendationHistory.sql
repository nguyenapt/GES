...
where
		I_GesCaseReports_Audit.NewI_GesCaseReportStatuses_Id IS NOT NULL 
GROUP BY I_GesCaseReports_Audit.I_GesCaseReports_Id ,
        I_GesCaseReports_Audit.NewI_GesCaseReportStatuses_Id

OPEN curR

FETCH NEXT FROM curR INTO @GesCaseReportsId, @RecommentdationId, @AuditDatetime
...
BEGIN
		...
		VALUES  ( NEWID() , -- GesCaseReports_Audit_Details - uniqueidentifier
		          ...
		          N'NewI_GesCaseReportStatuses_Id' , -- ColumnName - nvarchar(150)
		          N'' , -- OldValue - nvarchar(max)
		          CAST(@RecommentdationId AS NVARCHAR(max)) , -- NewValue - nvarchar(max)
